...
function postWorkerMessages(json) {
    ...
    demuxDecodeWorker.addEventListener("message", function (e) {
        ...
        // Gérer le message de reconnexion
        if (e.data.hasOwnProperty('reconnection')) {
            console.log("🔄 RECONNEXION DÉTECTÉE PAR LE THREAD PRINCIPAL");
            
            // Recalculer le zoom en fonction des dimensions actuelles
            zoom = Math.max(1, window.innerHeight / height);
            
            // Réappliquer la transformation au canvas
            canvasElement.style.transform = "scale(" + zoom + ")";
            
            console.log("🔎 ZOOM RECALCULÉ APRÈS RECONNEXION: " + zoom);
            
            // Demander une nouvelle keyframe pour s'assurer que l'image est à jour
            demuxDecodeWorker.postMessage({action: "REQUEST_KEYFRAME"});
            
            // Informer l'utilisateur
            warningElement.style.display = "block";
            logElement.style.display = "none";
            warningElement.innerText = "Reconnexion détectée, ajustement de l'affichage...";
            setTimeout(function() {
                warningElement.style.display = "none";
                logElement.style.display = "block";
            }, 3000);
