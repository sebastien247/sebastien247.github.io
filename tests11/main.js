...
function checkCanvasState() {
    ...
    if (canvasTransferred) {
        ...
        // Toujours appliquer les dimensions CSS correctes, en gardant trace des changements
        const oldStyleWidth = canvasElement.style.width || "";
        const oldStyleHeight = canvasElement.style.height || "";
        const newStyleWidth = width + "px";
        const newStyleHeight = height + "px";
        
        if (oldStyleWidth !== newStyleWidth || oldStyleHeight !== newStyleHeight) {
            console.log("[DIAG] ⚠️ Correction des dimensions CSS:", 
                      oldStyleWidth + "x" + oldStyleHeight,
                      "→", newStyleWidth + "x" + newStyleHeight);
            canvasElement.style.width = newStyleWidth;
            canvasElement.style.height = newStyleHeight;
        }
        
        // Vérifier si le transform est appliqué et corriger si nécessaire
        ...
        // Vérifier le transform-origin et corriger si nécessaire
        ...
        // Logger l'état après les corrections CSS
...
function postWorkerMessages(json) {
    ...
    // Au lieu d'appeler transferControlToOffscreen directement ici, utiliser notre fonction
    // setupOffscreenCanvas qui gère correctement le cas
    setupOffscreenCanvas();

    // Déplacer ce code dans la fonction setupOffscreenCanvas pour éviter la duplication
    ...
    // Utiliser la variable canvasTransferred pour savoir si le canvas a déjà été transféré
    if (!canvasTransferred) {
        console.log("[DIAG] ⚠️ Le canvas n'a pas pu être transféré au worker.");
    }
...
function setupOffscreenCanvas() {
    try {
        ...
        // S'assurer que le canvas a les bonnes dimensions AVANT le transfert
        if (!canvasTransferred && (canvasElement.width !== width || canvasElement.height !== height)) {
            console.log("[DIAG] Correction des dimensions avant transfert:", 
                       canvasElement.width + "x" + canvasElement.height,
                       "→", width + "x" + height);
            canvasElement.width = width;
            canvasElement.height = height;
        ...
        // Envoyer l'offscreen au worker avec tous les paramètres nécessaires
        demuxDecodeWorker.postMessage({
            canvas: offscreen, 
            port: port, 
            action: 'INIT', 
        ...
        // IMPORTANT: Ceci est crucial car on ne peut plus modifier width/height directement après transfert
        canvasElement.style.width = width + "px";
        canvasElement.style.height = height + "px";
        canvasElement.style.transform = "scale(" + zoom + ")";
        canvasElement.style.transformOrigin = "top left";
        ...
        // En cas d'erreur, marquer comme non transféré pour permettre les tentatives futures
...
function initCanvas() {
    ...
    if (canvasTransferred) {
        console.log("[DIAG] Canvas déjà transféré, utilisation des styles CSS uniquement");
        // Après transfert, on ne peut modifier que le style, pas les dimensions intrinsèques
        canvasElement.style.width = width + "px";
        canvasElement.style.height = height + "px";
    } else {
        // Si le canvas n'est pas encore transféré, on peut modifier ses dimensions
        console.log("[DIAG] Canvas non transféré, initialisation complète");
        canvasElement.width = width;
        canvasElement.height = height;
    }
    
    // Appliquer le zoom et l'origine (ces styles peuvent toujours être modifiés)
    zoom = Math.max(1, window.innerHeight / height);
    canvasElement.style.transform = "scale(" + zoom + ")";
    canvasElement.style.transformOrigin = "top left";
    ...
    console.log("[DIAG] Canvas initialisé avec:", {
        ...
        "transform": canvasElement.style.transform,
        "transformOrigin": canvasElement.style.transformOrigin,
        "estTransféré": canvasTransferred
    });
}
