...
// MCU1 / QtCarBrowser (Tesla MCU1) runs a ~2015 WebKit that cannot parse the
// modern UI. Redirect its document requests to the transpiled legacy/ bundle;
// every other browser falls through to the modern index.html below. Only the
// HTML document varies by UA — express.static still serves /legacy/* verbatim.
app.get(['/', '/index.html'], (req, res, next) => {
  res.set('Vary', 'User-Agent');
  const ua = req.headers['user-agent'] || '';
  const isLegacy = /QtCarBrowser/i.test(ua) && !/Chrom(e|ium)/i.test(ua);
  if (isLegacy) {
    res.set('Cache-Control', 'no-cache');
    return res.redirect(302, '/legacy/index.html');
  }
  next();
});
