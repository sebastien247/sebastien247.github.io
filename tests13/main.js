...
function postWorkerMessages(json) {
    ...
    if (json.hasOwnProperty("resolutionChanged")) {
        ...
        // Réinitialisation complète du canvas au lieu d'un simple redimensionnement
        if (offscreen) {
            // Recréer le canvas pour éviter les artefacts d'état
            const oldCanvas = canvasElement;
            const parentNode = oldCanvas.parentNode;
            
            // Créer un nouveau canvas avec les mêmes attributs
            ...
            // Remplacer l'ancien canvas par le nouveau
            if (parentNode) {
            ...
            // Réinitialiser le contrôle pour le nouveau canvas
            offscreen = canvasElement.transferControlToOffscreen();
            
            // Informer le worker du nouveau canvas et des nouvelles dimensions
            demuxDecodeWorker.postMessage({
                canvas: offscreen, 
                action: "REINIT",
                width: width,
            ...
            // Demander un nouveau keyframe après avoir reconstruit le canvas
            demuxDecodeWorker.postMessage({action: "REQUEST_KEYFRAME"});
            
            // Afficher un message temporaire
            warningElement.style.display = "block";
            logElement.style.display = "none";
            warningElement.innerText = "Résolution ajustée à " + width + "x" + height + " - Réinitialisation complète";
        } else {
            // Fallback si offscreen n'est pas encore défini
